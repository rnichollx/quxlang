...
normally use allocation regions, while the JVM path uses the managed object
storage builtins.
...
See [Allocation regions](../language/allocation-regions.md),
[`NEW` and `DELETE`](../language/new-and-delete.md),
[Concurrency and per-thread storage](../language/concurrency-and-per-thread-storage.md),
and [Diagnostics and explicit failure](../language/diagnostics-and-failure.md).
