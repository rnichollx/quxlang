# Static Objects and Materialization

`STATIC` declares a read-only value whose initializer is evaluated during
compilation. At global scope, Quxlang must also be able to preserve that value
...
instead of the direct structural contract. `STRUCT SERIALOID` requests that
...
without the `SERIALOID` modifier. When materializing the value, Quxlang prefers
a `.CONSTRUCTOR` taking `@DESERIALIZE_INPUT_ITERATOR`; otherwise it
default-constructs the object and calls `.DESERIALIZE`.

See [Serialization and stringlike types](serialization-and-stringlike.md) for
...
// Rejected: the type explicitly opts out of static materialization.
...
See [Variables and storage duration](variables-and-storage.md) and
[Static evaluation](static-evaluation.md) for those local forms.
