...
void quxlang::convert_llvm_object(llvm::object::ObjectFile const& obj, std::function< void(quxlang::object_symbol) > callback)
{
    ...
    for (auto const& section : obj.sections())
    {
        ...
        if (!section_name_ex)
        {
            throw quxlang::compiler_bug("Section has no name");
        }
        ...
        if (!data_ex)
        {
            throw quxlang::compiler_bug("Section has no data");
        }
        ...
        for (auto const& sym : obj.symbols())
        {
            ...
            if (!sym_name_ex)
            {
                throw quxlang::compiler_bug("Symbol has no name");
            }
            ...
            if (!sym_section_ex)
            {
                throw quxlang::compiler_bug("Symbol has no section");
            }
...

            {
                ...
                throw quxlang::compiler_bug(errorMessage);
            }
            else
            {
            ...
            if (!symAddrEx)
            {
                throw quxlang::compiler_bug("Symbol has no address");
            }
        ...
        calc_symbol_positions(symbol_map.begin(), symbol_map.end(), data.size(),
                              [&](symbol_map_info_output const& output)
                              {
                                  ...
                                  if (output.position >= data.size())
                                  {
                                      ...
                                      //    throw quxlang::compiler_bug("Symbol position is out of range");
                                  }
                                  if (output.position_end > data.size())
                                  {
                                      // TODO: check if this is correct
                                      //    throw quxlang::compiler_bug("Symbol position_end is out of range");
                                  }
