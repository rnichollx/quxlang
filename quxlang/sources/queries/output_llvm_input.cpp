...
rpnx::querygraph::coroutine< quxlang::output_llvm_input_spec > quxlang::output_llvm_input_impl(llvm_output_query_input input)
{
    ...
    while (!pending_types.empty())
    {
        ...
        for (type_symbol const& type : round_types)
        {
            ...
            if (type.type_is< composite_type >() || type.type_is< subsymbol >() || type.type_is< subtag_type >() || type.type_is< instanciation_reference >() || type.type_is< readonly_constant >() || (type.type_is< builtin_symbol >() && is_builtin_enum_name(type.get_as< builtin_symbol >().name)))
            {
