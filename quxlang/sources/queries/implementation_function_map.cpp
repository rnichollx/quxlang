...
rpnx::querygraph::coroutine< quxlang::implementation_function_map_spec > quxlang::implementation_function_map_impl(type_symbol input)
{
    ...
    for (interface_slot const& slot : slots)
    {
        ...
        {
            throw semantic_compilation_error("Implementation is missing interface function: " + slot.key.name);
        }
        ...
        if (!inst.has_value())
        {
            throw semantic_compilation_error("Implementation function does not match interface slot: " + slot.key.name);
        }
        ...
        {
            throw semantic_compilation_error("Implementation function return type does not match interface slot: " + slot.key.name);
        }
