...
{
    rpnx::cow< llvm_backend::llvm_preoptimized_unit > const& preoptimized = co_await rpnx::querygraph::request< llvm_preoptimize_query >(std::move(input));
    llvm_backend::llvm_backend backend;
    co_return backend.postoptimize(preoptimized.get());
}
