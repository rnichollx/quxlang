...
rpnx::querygraph::coroutine< quxlang::target_cortado_backend_options_spec > quxlang::target_cortado_backend_options_impl(std::monostate)
{
    target_configuration const& target_config = co_await rpnx::querygraph::request< target_configuration_query >(std::monostate{});
    co_return target_config.cortado_options;
}
