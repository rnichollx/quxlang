...
rpnx::querygraph::coroutine< quxlang::templex_builtins_spec > quxlang::templex_builtins_impl(type_symbol input)
{
    ...
    if (!is_builtin_ibc_template_name(builtin.name) && !is_builtin_allocator_name(builtin.name) && !is_builtin_atomic_templex_name(builtin.name) && builtin.name != "__DEFERRED")
    {
        co_return results;
    }
