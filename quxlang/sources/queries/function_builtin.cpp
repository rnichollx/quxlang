...
rpnx::querygraph::coroutine< quxlang::function_builtin_spec > quxlang::function_builtin_impl(temploid_reference input)
{
    ...
    {
        return typeis< numeric_literal_type >(type) || typeis< string_literal_type >(type) || type.type_is< int_type >() || type.type_is< float_type >() || type.type_is< bool_type >() || type.type_is< procedure_type >() || type.type_is< ptrref_type >() || type.type_is< array_type >() || type.type_is< byte_type >() || type.type_is< readonly_constant >() || type.type_is< constexpr_proxy >() || type.type_is< address_type >() || type.type_is< type_index_type >() || is_atomic_type(type);
    };
    ...
    if (parent_is_fusion && (member.name == "OPERATOR??"))
    {
        co_return builtin_function_kind::builtin_special;
    }
