...
rpnx::querygraph::coroutine< quxlang::have_nontrivial_member_ctor_spec > quxlang::have_nontrivial_member_ctor_impl(type_symbol input)
{
    ...
    if (class_is_builtin)
    {
        if (typeis< array_type >(input))
        {
            ...
            co_return co_await rpnx::querygraph::query_request< have_nontrivial_member_ctor_query >(element_type);
        }
        co_return false;
    }
    ...
    {
        auto field_ctor = co_await rpnx::querygraph::query_request< class_default_ctor_query >(field.type);
        if (field_ctor)
        {
            co_return true;
        }
