...
rpnx::querygraph::coroutine< quxlang::antestatal_static_value_spec > quxlang::antestatal_static_value_impl(type_symbol input)
{
    ...
    {
        throw std::logic_error("requested antestatal value for a non-antestatal static: " + quxlang::to_string(input));
    }

    auto symboid = co_await rpnx::querygraph::request< symboid_query >(input);
    if (!typeis< ast2_variable_declaration >(symboid))
    {
        throw std::logic_error("antestatal static symbol is not a variable: " + quxlang::to_string(input));
    }

    auto const& decl = as< ast2_variable_declaration >(symboid);
    if (!decl.init_expr.has_value())
    {
        throw std::logic_error("antestatal STATIC requires a := initializer: " + quxlang::to_string(input));
    }
    if (!decl.init_args.empty())
    {
        throw std::logic_error("antestatal STATIC initializer argument lists are not implemented: " + quxlang::to_string(input));
    }
