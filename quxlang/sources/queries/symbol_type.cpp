...
rpnx::querygraph::coroutine< quxlang::symbol_type_spec > quxlang::symbol_type_impl(type_symbol input)
{
   // auto type_str = to_string(input);
    ...
    if (typeis< builtin_symbol >(input))
    {
        ...
        if (builtin.name == "POLYMORPHIC_BASE" || is_builtin_atomic_access_mode_name(builtin.name) || is_builtin_enum_name(builtin.name))
        {
            co_return symbol_kind::class_;
        }
    ...
    if (typeis< instanciation_reference >(input))
    {
       ...
       else if (typeis< ast2_asm_procedure_declaration >(selected_ast) || typeis< ast2_extern_procedure >(selected_ast))
       {
          co_return symbol_kind::funtanoid;
       }
    ...
    else if (typeis< null_type >(input) || typeis< numeric_literal_type >(input) || typeis< string_literal_type >(input) || typeis< bool_type >(input) || typeis< int_type >(input) || typeis< float_type >(input) || typeis< procedure_type >(input) || typeis< ptrref_type >(input) || typeis< nvalue_slot >(input) || is_ref(input) || typeis< dvalue_slot >(input) || typeis< byte_type >(input) || typeis< initguard_type >(input) || typeis< initguard_lock_type >(input) || typeis< constexpr_proxy >(input) || typeis< thistype >(input) || typeis< readonly_constant >(input) || typeis< address_type >(input) || typeis< type_index_type >(input) || typeis< size_type >(input) || typeis< array_type >(input) || typeis< attached_type_reference >(input) || typeis< storage >(input) || typeis< aligned_storage >(input) || typeis< virtual_storage >(input))
    {
        co_return symbol_kind::class_;
    }
