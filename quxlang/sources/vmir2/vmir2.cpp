...
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_temporary(type_symbol type)
{
    local_index slot_id = local_index(slots.size());
    slots.push_back(vm_slot{.type = std::move(type), .kind = slot_kind::local});
    return slot_id;
}
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_variable(type_symbol type, std::string name)
{
    local_index slot_id = local_index(slots.size());
...
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_binding(local_index idx, type_symbol type)
{
    local_index slot_id = local_index(slots.size());
    slots.push_back(vm_slot{.type = type, .binding_of = idx, .kind = slot_kind::binding});
    return slot_id;
}
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_positional_argument(type_symbol type, std::optional< std::string > name)
{
    local_index slot_id = local_index( slots.size());
    slots.push_back(vm_slot{.type = type, .name = name, .kind = slot_kind::positional_arg});
    return slot_id;
}
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_named_argument(std::string apiname, type_symbol type, std::optional< std::string > varname)
{
    local_index slot_id = local_index(slots.size());
...
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_numeric_literal(std::string value)
{
...
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::create_bool_literal(bool value)
{
    // TODO Check if this literal already exists.
    local_index slot_id = local_index(slots.size());
...
quxlang::vmir2::local_index quxlang::vmir2::slot_generation_state::index_binding(local_index idx)
{
...
quxlang::type_symbol quxlang::vmir2::executable_block_generation_state::current_type(local_index idx)
{
...
bool quxlang::vmir2::executable_block_generation_state::slot_alive(local_index idx)
{
    return current_slot_states.at(idx).alive;
}
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_temporary(type_symbol type)
{
    auto idx = slots->create_temporary(type);
    return idx;
}
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_variable(type_symbol type, std::string name)
{
    return slots->create_variable(type, name);
}
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_binding(local_index idx, type_symbol type)
{
    return slots->create_binding(idx, type);
}
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_positional_argument(type_symbol type, std::optional< std::string > label_name)
{
...
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_named_argument(std::string interface_name, type_symbol type, std::optional< std::string > label_name)
{
...
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_numeric_literal(std::string value)
{
...
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::create_bool_literal(bool value)
{
...
quxlang::vmir2::local_index quxlang::vmir2::executable_block_generation_state::index_binding(local_index idx)
{
    return slots->index_binding(idx);
}
std::optional< quxlang::vmir2::local_index > quxlang::vmir2::executable_block_generation_state::local_lookup(std::string name)
{
    ...
    {
        if (slots->slots[i].name.has_value() && slots->slots[i].name.value() == name && current_slot_states[local_index(i)].alive)
        {
...
void quxlang::vmir2::frame_generation_state::generate_jump(std::size_t from, std::size_t to)
{
    ...
    block(from).block.terminator = vmir2::jump{.target = block_index(to)};
...
void quxlang::vmir2::frame_generation_state::generate_branch(std::size_t condition, std::size_t from, std::size_t true_branch, std::size_t false_branch)
{
    ...
    block(from).block.terminator = vmir2::branch{.condition = local_index(condition), .target_true = block_index(true_branch), .target_false = block_index(false_branch)};
    // TODO: Check value lifetimes here.
}
...
std::optional< quxlang::vmir2::local_index > quxlang::vmir2::frame_generation_state::lookup(std::size_t block_id, std::string name)
{
...
quxlang::vmir2::functanoid_routine2 quxlang::vmir2::frame_generation_state::get_result()
{
    ...
    result.local_types = slots.slots;
