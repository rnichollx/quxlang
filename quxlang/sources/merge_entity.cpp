...
void quxlang::merge_entity(ast2_symboid& destination, declaroid const& source)
{
    ...
    else if (typeis< ast2_test >(source))
    {
        if (!typeis< std::monostate >(destination))
        {
            throw quxlang::semantic_compilation_error("Cannot merge test into already existing entity");
        }

        destination = as< ast2_test >(source);
    }
    ...
    else if (typeis< ast2_extern_procedure >(source))
    {
        if (!typeis< std::monostate >(destination))
        {
            throw quxlang::semantic_compilation_error("Cannot merge extern_procedure into already existing entity");
        }
