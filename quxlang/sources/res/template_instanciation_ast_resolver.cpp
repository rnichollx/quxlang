...
auto quxlang::template_instanciation_ast_resolver::co_process(compiler* c, input_type input) -> co_type
{
    ...
    calltype templ_instanciation_args;
    ...
    for (ast2_template_declaration const& decl : templ_ast.templates)
    {
        ...
        auto can_use_this_template = co_await *c->lk_overload_set_instanciate_with(overload_set_instanciate_with_q{.overload = temploid_formal_paratype{.call_parameters = templ_instanciation_args}, .call = decl_args});
