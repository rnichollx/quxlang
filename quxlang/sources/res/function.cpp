...
QUX_CO_RESOLVER_IMPL_FUNC_DEF(list_builtin_constructors)
{
    ...
    if (typeis< int_type >(input) || input.type_is< bool_type >() || input.type_is< pointer_type >())
    {
        ...
        result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", create_nslot(input)}, {"OTHER", make_cref(input)}}}}, .return_type = void_type{}});
        if (input.type_is< int_type >())
        {
            result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", create_nslot(input)}, {"OTHER", numeric_literal_reference{}}}}}, .return_type = void_type{}});
        }
        result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", create_nslot(input)}}}}, .return_type = void_type{}});
        co_return (result);
    }
    ...
    {
        result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = intertype{.named = {{"THIS", create_nslot(input)}}}}, .return_type = input});
        co_return result;
    }

    if (should_autogen_copy_constructor)
    {
        result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = intertype{.named = {{"THIS", create_nslot(input)}, {"OTHER", make_cref(input)}}}}, .return_type = void_type{}});
    }
...
QUX_CO_RESOLVER_IMPL_FUNC_DEF(list_builtin_functum_overloads)
{
    ...
    if (typeis< submember >(functum))
    {
        ...
        if (name.starts_with("OPERATOR"))
        {
            ...
            if (assignment_operators.contains(operator_name))
            {
                allowed_operations.insert(primitive_function_info{
                    .overload = temploid_ensig{.builtin = true,
                                                  .interface =
                                                      intertype{
            ...
            {
                allowed_operations.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true,
                                                                                                .interface =
                                                                                                    intertype{
                                                                                                        .named = {{"THIS", parent}, {"OTHER", parent}},
                                                                                                    }},
            ...
            if (typeis< numeric_literal_reference >(parent) && arithmetic_operators.contains(operator_name))
            {
                ...
                allowed_operations.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", numeric_literal_reference{}}, {"OTHER", numeric_literal_reference{}}}}}, .return_type = numeric_literal_reference{}});
                co_return (allowed_operations);
            }

            if (typeis< pointer_type >(parent) && operator_name == rightarrow_operator)
            {
                allowed_operations.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", parent}}}}, .return_type = make_mref(remove_ptr(parent))});
            }
        ...
        else if (name == "CONSTRUCTOR")
        {
            ...
            {
                std::set< primitive_function_info > result;
                result.insert(primitive_function_info{.overload = temploid_ensig{.builtin = true, .interface = {.named = {{"THIS", make_mref(parent)}}}}, .return_type = void_type{}});
                co_return (result);
            }
    }
