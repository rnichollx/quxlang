...
QUX_CO_RESOLVER_IMPL_FUNC_DEF(class_builtin)
{
    ...
    if (typeis< int_type >(input_type) || typeis< bool_type >(input_type) || typeis< pointer_type >(input_type) || typeis< nvalue_slot >(input_type) || is_ref(input_type) || typeis<numeric_literal_reference>(input) || typeis<dvalue_slot>(input_type))
    {
        co_return true;
    }
    else
