...
namespace quxlang
{
    namespace vmir2
    {
        ...
        /** Recovers the exact typed storage that directly contains one object. */
        struct get_underyling_storage
        {
            local_index object_pointer;
            type_symbol storage_type;
            local_index storage_pointer;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(get_underyling_storage, object_pointer, storage_type, storage_pointer);
        };
        ...
        /** Loads the active dynamic TYPE_INDEX; a null or invalid source has undefined behavior. */
        struct struct_dynamic_type
        {
            local_index source;
            local_index result;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(struct_dynamic_type, source, result);
        };
        ...
        /** Reads a value from external memory. Runtime-only. */
        struct ibc_load
        {
            local_index address;
            local_index result;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(ibc_load, address, result);
        };

        /** Writes a value to external memory. Runtime-only. */
        struct ibc_write
        {
            local_index address;
            local_index value;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(ibc_write, address, value);
        };

        /** Forms a typed pointer into external memory. Runtime-only. */
        struct ibc_pun
        {
            local_index address;
            local_index result;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(ibc_pun, address, result);
        };

        /** Exposes a pointer address at a binary interface. Runtime-only. */
        struct ibc_getaddr
        {
            local_index pointer;
            local_index result;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(ibc_getaddr, pointer, result);
        };
        ...
        /** Produces the linked-output identity assigned to one canonical type. */
        struct load_type_index
        {
            type_symbol indexed_type;
            local_index result;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(load_type_index, indexed_type, result);
        };
        ...
        /** Transfers an owned value into uninitialized storage and ends its source lifetime. */
        struct relocate_value
        {
            local_index source;
            local_index target;

            QUXLANG_WITH_SOURCE_LOCATION_METADATA(relocate_value, source, target);
        };
        ...
        /** Computes ADDRESS % SZ as an unsigned SZ remainder; the divisor must be nonzero. */
        struct address_mod
        {
            local_index address;
            local_index divisor;
            local_index result;
            QUXLANG_WITH_SOURCE_LOCATION_METADATA(address_mod, address, divisor, result);
        };

        /** Compares two TYPE_INDEX values using canonical type-symbol ordering. */
        struct type_index_cmp
        {
            local_index a;
            local_index b;
            local_index result;
            QUXLANG_WITH_SOURCE_LOCATION_METADATA(type_index_cmp, a, b, result);
        };
        ...
        /** Terminates an execution path whose continuation is undefined behavior. */
        struct unreachable
        {
            QUXLANG_WITH_SOURCE_LOCATION_EMPTY_METADATA(unreachable);
        };
