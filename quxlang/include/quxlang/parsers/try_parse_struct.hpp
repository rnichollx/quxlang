...
{
    /** Parses a STRUCT or IBC_STRUCT declaration when the current token begins one. */
    inline std::optional< ast2_struct_declaration > try_parse_struct(parsing_context& ctx)
    {
        ...
        std::optional< std::string_view > const keyword = skip_keyword_if_one_of(pos, end, {"IBC_STRUCT", "STRUCT"});
