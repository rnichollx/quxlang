...
namespace quxlang::parsers
{
    ...
    std::optional< ast2_struct_declaration > try_parse_struct(parsing_context& ctx);
    std::optional< ast2_union_declaration > try_parse_union_declaration(parsing_context& ctx);
    std::optional< ast2_variant_declaration > try_parse_variant_declaration(parsing_context& ctx);
    ...
    inline std::optional< declaroid > try_parse_declaroid(parsing_context& ctx)
    {
        ...
        output = try_parse_struct(ctx);
        if (output)
        {
            return std::move(output);
        }
        output = try_parse_union_declaration(ctx);
        if (output)
        {
            return std::move(output);
        }
        output = try_parse_variant_declaration(ctx);
        if (output)
        {
            return std::move(output);
        }
