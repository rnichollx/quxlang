...
namespace quxlang::parsers
{
    ...
    namespace detail
    {
        ...
        inline std::optional< conversion_mode > try_parse_conversion_mode(parsing_context& ctx)
        {
            ...
            std::optional< std::string_view > keyword = skip_keyword_if_one_of(pos, end, {"EXPLICIT", "REINTERPRET", "PARTIAL", "ASSUME", "CHECKED", "APPROXIMATE", "DYNAMIC", "UNCHECKED_STATIC_DOWNCAST"});
    ...
    next_value:
        ...
        {
            expression_address_launder_escape_alloc_region launder_expr;
            skip_whitespace_and_comments(pos, end);
            launder_expr.pointer = parse_expression_impl(ctx);
            *value_bind_point = std::move(launder_expr);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "ADDRESS_LAUNDER_DISCOVER_EXISTING"))
        {
            expression_address_launder_discover_existing launder_expr;
            skip_whitespace_and_comments(pos, end);
            launder_expr.address = parse_expression_impl(ctx);
            skip_whitespace_and_comments(pos, end);
            if (!skip_keyword_if_is(pos, end, "TO"))
            {
                throw syntax_compilation_error("Expected 'TO' after ADDRESS_LAUNDER_DISCOVER_EXISTING <expr>");
            }
            skip_whitespace_and_comments(pos, end);
            auto to_type = try_parse_type_symbol(ctx);
            if (!to_type)
            {
                throw syntax_compilation_error("Expected type after ADDRESS_LAUNDER_DISCOVER_EXISTING <expr> TO");
            }
    ...
    next_operator:
        ...
        else if (skip_keyword_if_is(pos, end, "AS"))
        {
            ...
            if (!to_type)
            {
                throw syntax_compilation_error("Expected type after AS (optional EXPLICIT/REINTERPRET/PARTIAL/ASSUME/CHECKED/APPROXIMATE/DYNAMIC/UNCHECKED_STATIC_DOWNCAST)");
            }
