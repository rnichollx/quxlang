...
namespace quxlang::parsers
{
    ...
    next_value:
        ...
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_FITS"))
        {
            expression_numeric_literal_fits expr_fits;
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
            {
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_FITS");
            }
            expr_fits.literal_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
            {
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_FITS(<type>, ...)");
            }
            expr_fits.target_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
            {
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_FITS(<lit>, <target>)");
            }
            *value_bind_point = std::move(expr_fits);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_NEGATE"))
        {
            expression_numeric_literal_negate expr_neg;
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
            {
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_NEGATE");
            }
            expr_neg.operand_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
            {
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_NEGATE(<type>)");
            }
            *value_bind_point = std::move(expr_neg);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_ADD"))
        {
            expression_numeric_literal_binary_op expr_op;
            expr_op.op = "ADD";
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_ADD");
            expr_op.lhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_ADD(<type>, ...)");
            expr_op.rhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_ADD(<lhs>, <rhs>)");
            *value_bind_point = std::move(expr_op);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_SUBTRACT"))
        {
            expression_numeric_literal_binary_op expr_op;
            expr_op.op = "SUBTRACT";
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_SUBTRACT");
            expr_op.lhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_SUBTRACT(<type>, ...)");
            expr_op.rhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_SUBTRACT(<lhs>, <rhs>)");
            *value_bind_point = std::move(expr_op);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_MULTIPLY"))
        {
            expression_numeric_literal_binary_op expr_op;
            expr_op.op = "MULTIPLY";
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_MULTIPLY");
            expr_op.lhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_MULTIPLY(<type>, ...)");
            expr_op.rhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_MULTIPLY(<lhs>, <rhs>)");
            *value_bind_point = std::move(expr_op);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_DIVIDE"))
        {
            expression_numeric_literal_binary_op expr_op;
            expr_op.op = "DIVIDE";
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_DIVIDE");
            expr_op.lhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_DIVIDE(<type>, ...)");
            expr_op.rhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_DIVIDE(<lhs>, <rhs>)");
            *value_bind_point = std::move(expr_op);
            have_anything = true;
        }
        else if (skip_keyword_if_is(pos, end, "__NUMERIC_LITERAL_MODULUS"))
        {
            expression_numeric_literal_binary_op expr_op;
            expr_op.op = "MODULUS";
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, "("))
                throw syntax_compilation_error("Expected '(' after __NUMERIC_LITERAL_MODULUS");
            expr_op.lhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ","))
                throw syntax_compilation_error("Expected ',' after __NUMERIC_LITERAL_MODULUS(<type>, ...)");
            expr_op.rhs_type = try_parse_type_symbol(ctx).value();
            skip_whitespace_and_comments(pos, end);
            if (!skip_symbol_if_is(pos, end, ")"))
                throw syntax_compilation_error("Expected ')' after __NUMERIC_LITERAL_MODULUS(<lhs>, <rhs>)");
            *value_bind_point = std::move(expr_op);
            have_anything = true;
        }
