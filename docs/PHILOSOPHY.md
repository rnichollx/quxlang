...
Quxlang places a stronger emphasis on object semantics than C and C++. In C++, values of type `unsigned char` and `std::byte` can alias into the data
stored in other objects, data written as `std::int16_t` can be read back as `std::uint16_t`, and the destructors of primitive types are a no-op in C++. Quxlang lacks allowances for these kinds of wrong-type aliasing mechanics, and instead prioritizes strong object semantics. This doesn't mean that we cannot convert a value from I16 to U16, but it needs to be done using BIT_CAST or similar explicit mechanism, rather than relying on the language to allow data to be reinterpreted as a different "compatible" type.
The stronger object model allows for better aliasing optimization opportunities in the compiler, but requires more discipline from the programmer to load objects using the correct type of pointer.
Another feature missing from C++ is the ability to convert a struct and pointer to first member interchangably. In Quxlang, a pointer to a struct and a pointer to its first member are distinct types, and cannot be assumed to reside at the same address. If this behavior is required, PREFIX VAR must be explicitly declared and PREFIX_CAST can be then be used to convert between the two pointer types. In any situation where a member is not declared as a PREFIX VAR, creating a pointer or reference to the first member _drops provenance_ to the containing struct, even if they happen to share the same address in memory. Unlike most undefined behaviors, this rule may not be enforced by the constexpr VM, because of severe performance regressions that would be required to check pointer-provenance during constexpr evaluation, but can still lead to undefined behavior in native code due to optimizations.
