...
void rylang::merge_entity(rylang::entity_ast& destination, const rylang::entity_ast& source)
{
    ...
    {
        for (auto& x : std::get< function_entity_ast >(source.m_subvalue.get()).m_function_overloads)
        {
            std::get< function_entity_ast >(destination.m_subvalue.get()).m_function_overloads.push_back(x);
        }
    }
}